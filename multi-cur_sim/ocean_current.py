...
def build_current(center_name: Optional[str] = None) -> RealisticCurrentAdapter:
    ...
    if MapConfig.use_downloader:
        ...
        horiz_path, vert_path, origin, op_radius = downloader.download(
            center_name=center_name or list(downloader.centers.keys())[0]
        )
    else:
        horiz_path = str(MapConfig.cmems_horiz_file)
        vert_path  = str(MapConfig.cmems_vert_file)
        lon_c, lat_c, _, radius = MapConfig.operation_center[
            center_name or list(MapConfig.operation_center.keys())[0]
        ]
        origin = (lon_c, lat_c)

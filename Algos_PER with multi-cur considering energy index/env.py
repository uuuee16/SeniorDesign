...
class AUVEnv(gym.Env):
    ...
    def __init__(self, obstacle_name=None, target=None, current=None):
        ...
        self.target             = cfg.target.astype(np.float32).copy()
        ...
        # Action Space维度注释，[艏向速度, pitch角速度, yaw角速度]
    ...
    def _init_current(self, current):
        if not self.current_on:
            return None
        if self.current_mode == "synthetic" and current is None:
            return OceanCurrent()
        return current
    ...
    def _update_auv_dynamics(self, action: np.ndarray):
        ...
        pitch_rate = float(action[1]) * self.cfg.max_pitch_rate
        yaw_rate   = float(action[2]) * self.cfg.max_yaw_rate
    ...
    def _get_reward(self, pos0, action) -> tuple[float, dict]:
        ...
        # r_eng = math.cos(math.pi / 2.0 * float(action[1]))+ math.cos(math.pi / 2.0 * float(action[2]))
        r_eng = -(float(action[1]) ** 2 + float(action[2]) ** 2)

        reward = r_task + self.cfg.r_lambda_dist * r_dis + self.cfg.r_lambda_cur * r_cur + self.cfg.r_lambda_eng * r_eng
        part = {
            "r_task":     r_task,
            "r_distance": self.cfg.r_lambda_dist * r_dis,
            "r_current":  self.cfg.r_lambda_cur * r_cur,
            "r_consumption": self.cfg.r_lambda_eng * r_eng,
        }
    ...
    def reset(self, seed=None, options=None):
        ...
        while True:
            ...
            safe = True
            for (ox, oy, oz, r) in self.obstacles:
                dist_start = np.linalg.norm(self.pos - np.array([ox, oy, oz], dtype=np.float32))
                dist_target = np.linalg.norm(self.target - np.array([ox, oy, oz], dtype=np.float32))
                if dist_target < (r + 2.0) or dist_start < (r + 2.0):
                    safe = False
                    break
            if not safe:
                continue
    ...
    def step(self, action):
        ...
        reward, part = self._get_reward(pos0, action)

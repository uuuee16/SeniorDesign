    ...
    if args.train:
        ...
        while frame_idx < max_frames:           #小于最大步数，就继续训练
            ...
            for step in range(max_steps):
                ...
                # 合并 terminated 和 truncated 为 done (只要有一个为 True，episode 就结束了)
                done = terminated or truncated
                # 后续代码保持不变 (注意：如果 done 是 bool，你后面有转为 1/0 的操作，那也是可以的)
                next_state = next_state.astype(np.float32)
                env.render()
                done = 1 if done else 0  # 你的代码逻辑需要将 bool 转为 int
